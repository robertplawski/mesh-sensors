...
try:
    ...
    # "20,868,Hello World"
        ...
        def send_bytes(self, address: int, frequency: int, payload: bytes):
            offset_frequence = frequency - (850 if frequency>850 else 410)
            data = bytes([address>>8]) + bytes([address&0xff]) + bytes([offset_frequence]) + bytes([self.addr>>8]) + bytes([self.addr&0xff]) + bytes([self.offset_freq]) + payload 
            self.send(data)
            time.sleep(0.5)
        ...
        def receive(self):
            if self.ser.inWaiting() > 0:
                ...
                return { "payload": payload, "rssi":rssi, "node_id":node_id, "frequency": frequency  }
    ...
    print("[ERROR] RASPBERRY PI LORA HAT NOT AVAILABLE")
    class sx126x:
        def __init__(self,serial_num,freq,addr,power,rssi,air_speed=2400,\
                     net_id=0,buffer_size = 240,crypt=0,\
                     relay=False,lbt=False,wor=False):
